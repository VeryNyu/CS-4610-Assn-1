...
export function SearchableList({list}: ListProps) {
    ...
    // const filteredList = list.results.intersection(new Set(
    //     Array.from(list.results).filter((item: string) =>
    //         item.toLowerCase().includes(searchTerm.toLowerCase())
    // )));
    console.log(list);
    return (
        ...
        //             >
        //                 <Link
        //                     href={`/${list.root}/${item.name}`}
        //                     className="text-blue-600 hover:underline dark:text-blue-400"
        //                 >
        //                     {capitalize(item.name, list.root)}
        //                 </Link>
        //             </li>
        //         ))}
        //     </ul>
